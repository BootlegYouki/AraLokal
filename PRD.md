...
**Target Environment:** Philippine Public Elementary Schools (DepEd Grades 3–6), Rural Campuses, and Zero-Internet Classrooms  
...
flowchart TD
    ...
    subgraph HubServer["Local Hub (Teacher Laptop / School PC) - Tauri + Rust / Node Core"]
        ...
        CentralDB[("🗄️ Central SQLite DB<br/>• Authoritative Store<br/>• Gradebook Exporter (.xlsx/.csv)")]
    end
...
sequenceDiagram
    ...
    participant Hub as 🖥️ Local Hub Server
    ...
    Note over Pupil: ⏱️ Visual Countdown Timer Running (Green ➔ Yellow ➔ Red)
...
* **Socratic Persona:** "AraLokal AI", a friendly, patient, and encouraging guide adapted for Filipino elementary pupils (Grades 3 to 6).
...
* **Bilingual System Prompt Template:**
  ```text
  You are "AraLokal AI", a friendly and patient Socratic learning guide for Filipino elementary pupils (Grades 3 to 6).
...
sequenceDiagram
    ...
    participant Hub as 🖥️ Local Hub Server (Central DB)
